 ...
 *  Contains the enumeration of the states the system can take
 */
 ...
 *  This struct contains the memory for the state machine
 *
 *  Detailed description
 *  todo:
 */
typedef struct _emailValidator_t {
    states_t state;
    char *str;
    unsigned long pos;
...
}emailValidator_t /* optional variable list */;
...
void em_local_part_dot(emailValidator_t *data);
void em_domain_start(emailValidator_t *data);
void em_domain_run(emailValidator_t *data);
void em_domain_dot(emailValidator_t *data);
...
int isEmail(char *str)
{
    ...
    typedef void (*fptr)( emailValidator_t *data );
    /* Init the struct with initial data */
    emailValidator_t data = {
        .state = st_init,
        .str = str,
        .pos = 0,
        .length = strlen(str),
